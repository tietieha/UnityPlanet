    ...
    public class HUDTitleRender
    {
        ...
        void ReleaseCmmmandBuffer()
        {
            if (m_cmdBuffer != null)
            {
                if(m_renderCameara != null)
                    m_renderCameara.RemoveCommandBuffer(CameraEvent.AfterForwardAlpha, m_cmdBuffer);  // 挂在AfterForwardAlpha可以与物体做遮挡
                m_cmdBuffer.Clear();
        ...
        void FillMeshRender()
        {
            ...
            if (m_cmdBuffer.sizeInBytes > 0)
            {
                ...
                //caMain.AddCommandBuffer(CameraEvent.AfterForwardAlpha, m_cmdBuffer);  // 挂在AfterForwardAlpha可以与物体做遮挡
            }
        }
    }
};
